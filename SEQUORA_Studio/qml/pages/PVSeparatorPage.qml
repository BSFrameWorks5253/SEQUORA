...
ScrollView {
    ...
    ColumnLayout {
        width: Math.min(1240, Math.max(320, root.availableWidth - 36))
        ...
        Rectangle {
            ...
            RowLayout {
                ...
                Rectangle {
                    ...
                    color: root.theme.isDark ? "#2A1F14" : "#FEF3C7"
                    border.color: root.theme.isDark ? "#B45309" : "#FDE68A"
                    border.width: 1
                ...
                ColumnLayout {
                    ...
                    Text {
                        text: "High-speed Photo (P) vs Video (V) classification, folder segregation, and payload analysis."
                        font.pixelSize: 12
        ...
        GridLayout {
            ...
            Rectangle {
                Layout.fillWidth: true
                Layout.preferredWidth: root.availableWidth > 980 ? 460 : root.availableWidth
                ...
                ColumnLayout {
                    ...
                    Rectangle {
                        ...
                        gradient: Gradient {
                            ...
                            GradientStop { position: 0.0; color: (root.isScanning || root.isProcessing) ? root.theme.surface2 : (btnActionHov.containsMouse ? "#9E80F5" : root.theme.accent) }
                            GradientStop { position: 1.0; color: (root.isScanning || root.isProcessing) ? root.theme.surface2 : (btnActionHov.containsMouse ? "#8153EB" : "#7240E5") }
                        }
                        opacity: (root.sourceDir.trim() !== "" && root.destDir.trim() !== "") ? 1.0 : 0.45
                        enabled: (root.sourceDir.trim() !== "" && root.destDir.trim() !== "") && !root.isScanning && !root.isProcessing

                        border.color: (root.sourceDir.trim() !== "" && root.destDir.trim() !== "") ? Qt.rgba(1, 1, 1, 0.15) : "transparent"
                        border.width: 1
                        ...
                        RowLayout {
                            ...
                            Text {
                                text: root.isScanning ? "Analyzing Media Pipeline..." : (root.isProcessing ? "Extracting Photo Assets..." : "Start Photo Extraction")
                                font.pixelSize: 13
                                font.weight: Font.Bold
                                color: (root.sourceDir.trim() !== "" && root.destDir.trim() !== "") ? "#FFFFFF" : root.theme.textMuted
                            }
                        }
